...
class Statistics:
    ...
    async def statschannel(self, context, channel: discord.Channel=None):
        ...
        Example: *statschannel #statistics
        """
        if channel:
    ...
    async def embed_statistics(self):
        ...
        stream = len([e.status for e in self.bot.get_all_members()
                      if e.game is not None and e.game.url is not None]) 
        em = discord.Embed(description=u'\u2063\n', color=discord.Color.blue())
        avatar = self.bot.user.avatar_url if self.bot.user.avatar else self.bot.user.default_avatar_url
        em.set_author(name='Trinity\'s Stats'.format(stats['name']), icon_url=avatar)

        em.add_field(name="Developer", value="<@153286414212005888>")
        ...
        em.add_field(name="Users", value="**{}** Online <:online:353413480793112576>\n**{}** Idle <:away:353413480310898688>\n**{}** Dnd <:dnd:353413480449310730>\n**{}** Streaming <:streaming:353413481069805568>\n**{}** Offline <:offline:353413480528871425>\n\nTotal Users: **{}**".format(online, idle, dnd, stream, offline, str(stats['users'])))
        em.add_field(name='**Servers**', value=stats['total_servers'])

        em.add_field(name='**Channels**', value='<:channel:365738833398202368> Text Channels: **{}**\n<:voice:365738858555637760> Voice Channels: **{}**\n\nTotal Channels: **{}**'.format(str(stats['text_channels']), str(stats['voice_channels']), str(stats['channels'])))
        ...
        em.set_footer(text='API version {}'.format(discord.__version__), icon_url='https://cdn.discordapp.com/attachments/317217370852687873/361293039664365570/images.jpg')
        return em
