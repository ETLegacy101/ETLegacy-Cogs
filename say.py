...
class say:
    ...
    async def whisper(self, ctx, user : discord.Member, text):
        ...
        try:
            ...
            log.debug("Can't send messages to **{}**. Probably he blocked me!".format(user))
        except (discord.errors.NotFound, discord.errors.InvalidArgument):
            log.debug("{} Not found.".format(user))
        else:
            reply = await self.bot.say("Message successfully sent :D")
