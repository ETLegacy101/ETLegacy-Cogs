...
class mute:
    ...
    async def cmute(self, ctx, user: discord.Member, duration: str=None, *, reason: str=None):
        ...
        if user.id in self.json[server.id]:
            msg = 'User was already Muted\nResetting their timer!'
        elif role in user.roles:
            msg = 'User was Muted but had no timer, Muting that user now!...'
        else:
            msg = 'I have succesfully muting that user! .'
    ...
    async def mute(self, ctx, user: discord.Member, duration: str=None, *, reason: str=None):
       ...
       Time Units ==> s,m,h,d.
        Example: *mute @Dumbass#4053 1.1h10m ***Stop Spamming YO!!***"""
        ...
        if user.id in self.json[server.id]:
            ...
            msg = 'I have succesfully muted that user'
    ...
    async def warn(self, ctx, user: discord.Member, *, reason: str=None):
        ...
        if reason is None: 
            msg = "hey, {} you are doing something wrong  ".format(user.mention, server.name)
        else:
    ...
    async def setup_role(self, server, quiet=False):
        ...
        if not role:
            if not (any(r.permissions.manage_roles for r in server.me.roles) and
                ...
                msg = "The **%s** **role Is inexistent**\n:raised_hand:***Creating it now wait up...***:raised_hand:\n" % self.role_name
                if not quiet:
    ...
    async def unmute(self, ctx, user: discord.Member):
        ...
        if role and role in user.roles:
            reason = 'Mute manually ended early by a mod/admin named => %s. ' % ctx.message.author
            if self.json[sid][user.id]['reason']:
                reason += self.json[sid][user.id]['reason']
            await self._unmute(user, reason)
            await self.bot.say('i have succesfully unmuted that user :D ')
        else:
